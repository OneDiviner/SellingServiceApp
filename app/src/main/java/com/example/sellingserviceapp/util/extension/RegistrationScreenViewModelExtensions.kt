...
fun RegistrationViewModel.sendCodeToVerificationRequest(authRepository: AuthRepository, stateUpdater: StateUpdater, tokenManager: TokenManager) {
    viewModelScope.launch {
        ...
        result.onSuccess { response ->
            ...
            tokenManager.saveToken(response.emailToken)
...
fun RegistrationViewModel.firstStepRegisterRequest(authRepository: AuthRepository, stateUpdater: StateUpdater) {
    viewModelScope.launch {
        ...
        result.onSuccess {
            ...
            when(userStatus) {
                "Not Active" -> {
                    ...
                    createVerificationEmailCodeRequest(authRepository, stateUpdater)
                    stateUpdater.nextButtonUpdater(ButtonState.Ready)
                    Log.d("FIRST_STEP_REGISTER_ERROR", "ERROR: ${error.message}, Status[$userStatus]")
                }
                "Active" -> {
